...
def create_job(request):
    ...
    if len(Category.objects.filter(category = request.POST['category_text'])) != 0:
        messages.error(request, "This category already exists! Try another!")
        return redirect('/add')
...
def update_job(request, job_id):
    ...
    if len(Category.objects.filter(category = request.POST['category_text'])) != 0:
        messages.error(request, "This category already exists, check again from list!")
        return redirect(f'/jobs/{job_id}/edit')

    if current_job.created_by_user != current_user:
        messages.error(request, "Job has not been created yet, Unable to edit it!")
        return redirect('/dashboard')
...
def done_job(request, job_id):
    ...
    if current_user not in current_job.added_users.all():
        messages.error(request, "Job has not been added! Unable to finish!")
        return redirect('/dashboard')
